...
export default function PrismComponent(props) {
  const [formatCode, setFormatCode] = React.useState(false);
    ...
    tokenloop: for (var token in grammar) {
      ...
      for (var j = 0; j < patterns.length; ++j) {
        ...
        for (
          var i = 0, pos = 0;
          i < strarr.length;
          pos += (strarr[i].matchedStr || strarr[i]).length, ++i
        ) {
          var str = strarr[i];
...
Token.reactify = function (o, language, parent, key) {
  ...
  return React.createElement(env.tag, env.attributes, env.content);
};
